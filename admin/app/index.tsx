...
//
// In the standalone admin app this IS the root screen:
//   - signed out            → <Redirect href="/sign-in" />
//   - signed in, not admin  → "Admins only" wall
//   - signed in + admin     → the console
...
export default function AdminScreen() {
  ...
  // Auth + session still resolving → splash.
  ...
  // Signed in but not an admin → restricted wall (with a way out).
  if (!isAdmin) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: t.colors.bg, alignItems: 'center', justifyContent: 'center', padding: 32, gap: 12 }}>
        ...
        <Pressable onPress={() => signOut()} hitSlop={10}>
          <Text style={{ color: t.colors.primary, fontWeight: '800', marginTop: 8 }}>Sign out</Text>
        </Pressable>
      </SafeAreaView>
    );
  }
