...
/**
 * Orders sitting in the dispatch pool — placed but not yet claimed by a rider.
 * Note: a *pure* rider cannot read the pool (RLS scopes orders to the assigned
 * rider only); this is non-empty for admins / dispatch, who hand orders out.
 */
...
export function useAvailableOrders() {
  ...
  const refresh = useCallback(async () => {
    const { data: ordered } = await supabase
      .from('orders')
      .select('id, status, landmark, driver_payload, total_dh, created_at')
