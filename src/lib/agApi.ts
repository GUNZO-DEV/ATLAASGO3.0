/**
 * AtlaasGo — Supabase-native implementation of handoff/AtlaasGo API.md.
 *
 * Same surface and field names as the handoff's `api-client.ts`, but instead of
 * hitting a REST service at api.atlaasgo.com it talks straight to Supabase
 * (Postgres + RPC + Realtime). A prototype screen can swap its `window.AG.*`
 * mocks for `agApi.*` 1:1 — the returned shapes match the spec exactly.
 *
 * Auth note: the spec describes phone OTP; AtlaasGo uses Clerk. We therefore
 * skip `/auth/*` (Clerk owns sign-in) and resolve `me.*` from the live Supabase
 * session that the Clerk→Supabase bridge already establishes.
 */
...
function orderRowToSpec(o: OrderRow): Order {
  ...
  return {
    ...
    address: {
      id: o.address_id ?? '',
      cityId: (o as OrderRow & { city?: string }).city ?? 'ifrane',
      label: o.landmark ?? '',
...
export const agApi = {
  ...
  orders: {
    ...
    tracking: async (id: string): Promise<Tracking | null> => {
      ...
      if (!o) return null;
      const order = o as OrderRow & { city?: string };
