...
import { BG, CARD, LINE, EMERALD, GLOW, CREAM, MUTED } from './ui';
import { RIDER_QUICK_REPLIES, useOrderChat, type OrderMessage } from '../../hooks/useOrderChat';

/**
 * Driver-side order chat (dark emerald cockpit). Same realtime thread the
 * customer sees — the rider sends as sender_role 'rider'. RLS already permits
 * the active rider, so this just mounts the UI.
 */
      ...
      <Pressable onPress={openMap} disabled={!isLocation}>
        <View
          style={{
            ...
            paddingVertical: 10,
            ...(isLocation
              ? { borderWidth: 1, borderColor: mine ? 'rgba(255,255,255,0.4)' : 'rgba(52,211,153,0.4)', borderStyle: 'dashed' }
              : {}),
          }}
        >
          {isLocation ? (
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <View style={{ width: 32, height: 32, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: mine ? 'rgba(255,255,255,0.2)' : 'rgba(52,211,153,0.12)' }}>
                <MapPin size={14} color={mine ? '#04140D' : GLOW} strokeWidth={2.5} />
              </View>
              <View style={{ marginLeft: 10 }}>
                <Text style={{ fontSize: 14, fontWeight: '700', color: mine ? '#04140D' : CREAM }}>
                  {message.body || 'My location'}
                </Text>
                <Text style={{ fontSize: 11, marginTop: 1, color: mine ? 'rgba(4,20,13,0.7)' : MUTED }}>Tap to open map</Text>
              </View>
            </View>
          ) : (
            <Text style={{ fontSize: 14.5, lineHeight: 20, color: mine ? '#04140D' : CREAM }}>{message.body}</Text>
          )}
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end', marginTop: 3, gap: 4 }}>
            <Text style={{ fontSize: 10, color: mine ? 'rgba(4,20,13,0.6)' : MUTED }}>{fmtTime(message.created_at)}</Text>
            {mine ? <CheckCheck size={13} color={message.read_at ? '#0A5E44' : 'rgba(4,20,13,0.5)'} /> : null}
          </View>
        </View>
      </Pressable>
    </View>
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: BG }} edges={['top', 'bottom']}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 18, paddingTop: 6, paddingBottom: 12, borderBottomWidth: 1, borderColor: LINE }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
            <View style={{ width: 40, height: 40, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(52,211,153,0.12)' }}>
              <MessageCircle size={18} color={GLOW} />
            </View>
        ...
        {!me ? (
          ...
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 8 : 0}>
            ...
            {error ? <Text style={{ fontSize: 12, paddingHorizontal: 18, paddingBottom: 4, color: '#F87171' }}>{error}</Text> : null}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 18, gap: 8, paddingVertical: 8 }} style={{ flexGrow: 0 }}>
              {RIDER_QUICK_REPLIES.map((q) => (
                <Pressable key={q} onPress={() => onSend(q)} disabled={sending}>
                  <View style={{ borderRadius: 999, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: CARD, borderWidth: 1, borderColor: 'rgba(52,211,153,0.3)', opacity: sending ? 0.5 : 1 }}>
                    <Text style={{ fontSize: 12, fontWeight: '700', color: GLOW }}>{q}</Text>
                  </View>
                </Pressable>
              ))}
            </ScrollView>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingTop: 2, paddingBottom: 6, borderTopWidth: 1, borderColor: LINE, gap: 8 }}>
              <Pressable onPress={sharePin} disabled={pinBusy}>
                <View style={{ width: 44, height: 44, borderRadius: 999, alignItems: 'center', justifyContent: 'center', backgroundColor: CARD, borderWidth: 1, borderColor: LINE, opacity: pinBusy ? 0.5 : 1 }}>
                  {pinBusy ? <ActivityIndicator size="small" color={EMERALD} /> : <MapPin size={17} color={GLOW} />}
                </View>
              </Pressable>
              <TextInput
                value={draft}
