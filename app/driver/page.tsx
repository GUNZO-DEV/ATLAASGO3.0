...
export default function DriverPage() {
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto flex flex-col gap-8">
        <div>
          <h1 className="text-2xl font-bold text-emerald-atlaasgo">Driver Dashboard</h1>
        ...
        {active.length > 0 && (
          <section className="flex flex-col gap-4">
            <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest">Active</h2>
            {active.map((order) => (
              <div
                key={order.id}
                className="bg-white rounded-2xl p-5 flex flex-col gap-4 border border-gold/40 shadow-[0_2px_12px_rgba(184,151,62,0.15)]"
              >
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="font-semibold text-gold">
                    {formatMAD(getDeliveryFee(order.zone))}
                  </span>
                  {order.phone && (
                    <a
                      ...
                      className="flex items-center gap-1.5 text-xs text-white bg-[#25D366] hover:bg-[#1ebe5d] px-3 py-1.5 rounded-lg font-medium transition"
                    >
        ...
        {/* Pending orders — only visible when online */}
        <section className="flex flex-col gap-4">
          <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest">Available</h2>
          {!isOnline ? (
            <div className="bg-white rounded-2xl p-8 text-center border border-gray-200 flex flex-col items-center gap-2">
              <span className="text-2xl">☕</span>
              <p className="text-gray-500 text-sm font-medium">You&apos;re on a break.</p>
              <p className="text-gray-400 text-xs">Toggle &quot;Active &amp; Ready&quot; to see new orders.</p>
            </div>
          ) : pending.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center text-gray-400 border border-gold/20">
            ...
            pending.map((order) => {
              ...
              return (
                <div
                  key={order.id}
                  className={`bg-white rounded-2xl p-5 flex items-center justify-between gap-4 border transition
                    ${isNew
                      ? "border-emerald-atlaasgo/60 animate-order-ping"
                      : "border-gold/30 shadow-[0_2px_12px_rgba(184,151,62,0.12)] hover:shadow-[0_4px_20px_rgba(184,151,62,0.25)]"
                    }`}
                >
                  <div className="flex flex-col gap-1 min-w-0">
                    ...
                    <div className="flex gap-3 text-xs text-gray-400">
                      ...
                      <span className="font-semibold text-gold">{formatMAD(getDeliveryFee(order.zone))}</span>
                    </div>
                  </div>
                  <button
